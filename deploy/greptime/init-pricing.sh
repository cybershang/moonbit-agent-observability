...
echo "==> Executing pricing SQL from ${SQL_FILE}..."
...
sed 's/--.*$//' "$SQL_FILE" | tr '\n' ' ' | tr ';' '\n' > /tmp/pricing-stmts.txt
while IFS= read -r stmt; do
    stmt=$(echo "$stmt" | sed 's/^[[:space:]]*//;s/[[:space:]]*$//')
    [ -z "$stmt" ] && continue
    sql "$stmt"
done < /tmp/pricing-stmts.txt

echo ""
echo "==> Done! Model pricing table initialized/updated."
echo ""
echo "   Verify with:"
echo "     mysql -h 127.0.0.1 -P 4002 -e 'SELECT * FROM model_pricing'"
echo ""
